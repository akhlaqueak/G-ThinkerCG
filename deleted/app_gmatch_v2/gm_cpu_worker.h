...
Graph_CPU query_graph_cpu;
Graph_CPU data_graph;
Plan plan;
GMPlan gmp;
Graph gpuG;
...
public:
    ...
    {
        GMTask *t = new GMTask();
        t->context.query_vertices_num = query_graph_cpu.getVerticesCount();
        t->context.cur_depth = 1;

        t->context.embedding = new ui[query_graph_cpu.getVerticesCount()];
        t->context.idx_embedding = new ui[query_graph_cpu.getVerticesCount()];
    ...
    virtual void compute(GMContext &context)
    {
        if (temp_buffer == NULL)
        {
            ...
            idx = new ui[query_graph_cpu.getVerticesCount()];
            idx_count = new ui[query_graph_cpu.getVerticesCount()];

            // allocate space for valid candidate 2-dimensional array
            valid_candidate_idx = new ui*[query_graph_cpu.getVerticesCount()];
        ...
        LFTJ(context.cur_depth, query_graph_cpu, edge_matrix, candidates, candidates_count, matching_order, context.embedding, 
    ...
    void LFTJ(int enter_depth, Graph_CPU &query_graph_cpu, Edges ***edge_matrix, ui **candidates,
                ui *candidates_count, ui *order, ui *embedding, ui *idx_embedding,
                ui **bn, ui *bn_count)
    {
        int cur_depth = enter_depth;
        int max_depth = query_graph_cpu.getVerticesCount();
