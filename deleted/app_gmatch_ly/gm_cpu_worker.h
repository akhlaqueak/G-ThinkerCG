...
Graph_CPU query_graph;

Graph_CPU data_graph_cpu;
...
public:
    ...
    {
        GMTask *t = new GMTask();
        t->context.query_vertices_num = query_graph.getVerticesCount();
        t->context.cur_depth = 1;

        t->context.embedding = new ui[query_graph.getVerticesCount()];
        t->context.idx_embedding = new ui[query_graph.getVerticesCount()];
    ...
    virtual void compute(GMContext &context)
    {
        if (temp_buffer == NULL)
        {
            ...
            visited_arr = new bool[data_graph_cpu.getVerticesCount()];
            memset(visited_arr, false, sizeof(bool)*data_graph_cpu.getVerticesCount());
        ...
        LFTJ(context.cur_depth, query_graph, edge_matrix, candidates, candidates_count, matching_order, context.embedding, 
    ...
    void LFTJ(int enter_depth, Graph_CPU &query_graph, Edges ***edge_matrix, ui **candidates,
