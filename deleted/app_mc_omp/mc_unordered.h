...
ui inplace_intersect(auto beg1, auto end1, auto beg2, auto end2)
{
    auto res = beg2;
    ui count=0;
    while (beg2 < end2){
...
public:
    ...
    void BK(bkVector &R, bkVector &P, bkVector &X, bkVector &Xp, auto st)
    {
        ...
        for (ui i = 0; i < P.size();)
        {
            ...
            ui xsz = inplace_intersect(nbrs, nbrs + nbr_count, X.begin(), X.end()); // result is stored in X; 
