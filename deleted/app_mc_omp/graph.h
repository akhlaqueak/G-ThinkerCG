...
private:
    ...
    void read_bin(const std::string&);
    void read_bin_degen(const std::string&);
...
**/
  ...
  void Graph::read_bin(const string &path)
  {
    ...
    fread(&tt, sizeof(ui), 1, fp);
    fread(&vertices_count_, sizeof(ui), 1, fp);
    fread(&edges_count_, sizeof(ui), 1, fp);
    
    std::cout << "vertex_count=" << vertices_count_ << ", edge_count=" << edges_count_ << std::endl;



    offsets_ = new long long[vertices_count_ +  1];
    offsets_[0] = 0;
    neighbors_ = new ui[edges_count_];
    labels_ = new ui[vertices_count_];
    labels_count_ = 0;

    
    ui* degree=new ui[vertices_count_];
    fread(degree, sizeof(ui), vertices_count_, fp);
    fread(neighbors_, sizeof(ui), edges_count_, fp);

    std::partial_sum(degree, degree+vertices_count_, offsets_+1);
    max_degree_ = *std::max_element(degree, degree+vertices_count_);
  ...
  void Graph::read_bin_degen(const string &path)
  {
    FILE *fp = fopen(path.c_str(), "r");
    if (nullptr == fp)
    {
    ...
    res += fread(&uintV_size, sizeof(size_t), 1, fp);
    res += fread(&uintE_size, sizeof(size_t), 1, fp);
    res += fread(&vertices_count_, sizeof(size_t), 1, fp);
    res += fread(&edges_count_, sizeof(size_t), 1, fp);
    ...
    res += fread(offsets_, uintE_size, vertices_count_ + 1, fp);
    res += fread(neighbors_, uintV_size, edges_count_, fp);
    
    for(ui i=0;i<vertices_count_; i++)
        if(offsets_[i+1]-offsets_[i]>max_degree_)
        max_degree_ = offsets_[i+1]-offsets_[i];

    
    // ui* degree=new ui[vertices_count_];
    // fread(degree, uintV_size, vertices_count_, fp);
    // fread(neighbors_, uintV_size, edges_count_, fp);
...
void Graph::loadGraphFromFile(std::string &file_path) {
    ...
    if(file_path.find(".bin")!=std::string::npos) return read_bin_degen(file_path);
