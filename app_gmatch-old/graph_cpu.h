...
private:
    ...
    void WriteDegreeDistributionToJson(const std::unordered_map<ui, ui>& degree_count, const std::string& filename);
...
public:
    ...
    void loadGraphFromCSR(ull *, ui *, ui n);
...
void getKCore(const Graph_CPU *graph, ui *core_table)
{
...
void Graph_CPU::BuildReverseIndex()
{
...
void Graph_CPU::BuildLabelOffset()
{
...
void Graph_CPU::WriteDegreeDistributionToJson(const std::unordered_map<ui, ui>& degree_count, const std::string& filename)
{
    std::vector<std::pair<ui, ui>> sorted_degrees(degree_count.begin(), degree_count.end());
    ...
    for (size_t i = 0; i < sorted_degrees.size(); ++i) {
        out << "  \"" << sorted_degrees[i].first << "\": " << sorted_degrees[i].second;
        if (i < sorted_degrees.size() - 1) out << ",";
        out << "\n";
    }
    out << "}\n";

    out.close();
    std::cout << "Degree distribution written to: " << filename << std::endl;
}

void Graph_CPU::loadGraphFromCSR(ull *row_ptrs, ui *cols, ui n)
{
    vertices_count_ = n;
    offsets_ = new ui[n + 1];
    edges_count_ = row_ptrs[n];
    neighbors_ = new ui[row_ptrs[n]];
    labels_ = new ui[n];
    max_degree_ = 0;

    std::fill(labels_, labels_ + n, 0);
    std::copy(row_ptrs, row_ptrs + n + 1, offsets_);
    std::copy(cols, cols + edges_count_, neighbors_);

    // Degree distribution map
    std::unordered_map<ui, ui> degree_count;

    for (ui i = 0; i < n; i++)
    {
        ui deg = row_ptrs[i + 1] - row_ptrs[i];
        if (deg > max_degree_)
            max_degree_ = deg;
...
//         ui* st = cols+row_ptrs[i];
//         ui* en = cols+row_ptrs[i+1];

//         auto it = std::lower_bound(st, en, i);
//         ui deg=0;

//         if (it != en){
//             deg = std::distance(it, en);
//             std::copy(it, en, neighbors_+offsets_[i]);
//         }
//         offsets_[i+1]=offsets_[i]+deg;
//         max_degree_=std::max(deg, max_degree_);
//     }
//     std::fill(labels_, labels_+n, 0);
//     edges_count_ = offsets_[n];
...
void Graph_CPU::loadGraphFromFile(std::string &file_path)
{
