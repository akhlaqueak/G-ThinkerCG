        ...
        while (i < (*(dd.read_count)))
        {
            ...
            for (int j = 0; j < wd.expansions[WIB_IDX]; j++)
            {
                ...
                for (index = LANE_IDX; index < wd.number_of_members[WIB_IDX]; index += WARP_SIZE) {
                    ld.vertices[index] = Brd.[wd.start[WIB_IDX] + index];
                }
                for (; index < wd.total_vertices[WIB_IDX] - 1; index += WARP_SIZE) {
                    ld.vertices[index + 1] = Brd.[wd.start[WIB_IDX] + index];
                }

                if (LANE_IDX == 0) {
                    ld.vertices[wd.number_of_members[WIB_IDX]] = Brd.[wd.start[WIB_IDX] + wd.total_vertices[WIB_IDX] - 1];
                }
                __syncwarp();
