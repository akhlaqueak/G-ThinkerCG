...
namespace STMatch {
  ...
  struct PatternPreprocessor {
    ...
    PatternPreprocessor(int gmatch_query_id) {
      read_gmatch_query(gmatch_query_id);
      preprocess_pattern();
    }
    ...
    void read_gmatch_query(int query_id) {
      const auto adj = make_gmatch_query_adjacency(query_id);
      if (adj.size() > PAT_SIZE) {
        throw std::invalid_argument("app_gmatch query exceeds STMatch PAT_SIZE");
      }

      pat.nnodes = static_cast<pattern_node_t>(adj.size());
      vertex_labels.assign(static_cast<size_t>(pat.nnodes) + 1, 1);
