...
mkdir build
cd build
cmake .. -DEGSM_CUDA_ARCHITECTURES=80
make
cd ..
```
...
For G-Thinker/app_gmatch inputs, EGSM also accepts a single-file CSR binary
data graph and numeric preset query id:

```shell
./build/EGSM -dg /path/to/data.bin -q 2
```

The helper script builds if needed and runs the same executable:

```shell
./run.sh -dg /path/to/data.bin -q 2
```
