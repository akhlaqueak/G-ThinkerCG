...
int Graph::LoadGraph(char* szgraph_file) // create 1-hop neighbors
{
	...
	for(int nvertex_no=0; nvertex_no<num_of_vertices; nvertex_no++)
	{
		...
		for(unsigned long long i=onehop_offsets[nvertex_no]; i<onehop_offsets[nvertex_no + 1]; i++)
		{
			if(onehop_neighbors[i]!=nvertex_no)
				mppadj_lists[nvertex_no][nlist_len++] = onehop_neighbors[i];
		}
		nbuf_pos += nlist_len;
		mppadj_lists[nvertex_no][0] = nlist_len-1;
		qsort(&mppadj_lists[nvertex_no][1], mppadj_lists[nvertex_no][0], sizeof(int), comp_int);
		if(nmax_deg < mppadj_lists[nvertex_no][0])
